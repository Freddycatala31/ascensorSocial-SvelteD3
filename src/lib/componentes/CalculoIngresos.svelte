<script>
  ...
  let width = 600; 
  let height = 300;
  let margin = { top: 20, right: 20, bottom: 40, left: 50 }; // Un poco más de margen abajo
...
<div class="chart-wrapper" bind:clientWidth={containerWidth}>
  <svg viewBox="0 0 {width} {height}" on:mouseleave={handleMouseLeave}>
    ...
    <g transform="translate({margin.left}, {margin.top})">
      ...
      <line x1="0" y1={innerHeight} x2={innerWidth} y2={innerHeight} stroke="#ccc" />
      ...
      {#if hoveredPoint}
        <line 
          x1={xScale(hoveredPoint.centil)} 
          x2={xScale(hoveredPoint.centil)} 
          y1={yScale(hoveredPoint.renta)} 
          y2={innerHeight} 
          stroke="#718096" 
          stroke-dasharray="4"
        />
        ...
        <line 
          ...
          x2={xScale(hoveredPoint.centil)} 
          y1={yScale(hoveredPoint.renta)} 
          y2={yScale(hoveredPoint.renta)} 
          stroke="#718096" 
          stroke-dasharray="4"
        />

        <circle 
          cx={xScale(hoveredPoint.centil)} 
          cy={yScale(hoveredPoint.renta)} 
          r="5" 
          fill="white" 
          stroke="#2b6cb0" 
          stroke-width="2"
        ...
        <g transform="translate({xScale(hoveredPoint.centil)}, {yScale(hoveredPoint.renta) - 75})">
          ...
          <rect 
            x={hoveredPoint.centil > 80 ? -145 : 5}
            y="0"
            width="140"
            height="60"
            rx="8" ry="8"
          ...
          <text 
            ...
            x={hoveredPoint.centil > 80 ? -20 : 20} 
            y="25" 
            font-weight="bold" 
            fill="#2d3748"
            font-size="16"
...
<style>
  ...
  text {
    pointer-events: none;
    /* Quitamos el text-shadow anterior porque ahora tenemos fondo blanco limpio */
  }
</style>